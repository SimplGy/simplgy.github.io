...
In this tutorial I'll show you how to build an [infinite scrolling mechanism](https://jsfiddle.net/yhvrcwog/2/embedded/result/) using Functional Reactive Programming techniques using [Bacon.js](https://github.com/baconjs/bacon.js).

Functional Reactive Programming, or FRP, is the [newish](http://hipsterdevstack.tumblr.com/post/39558331788/frp-yeah-we-were-doing-that-in-2012) [hotness](https://www.google.com/trends/explore#q=functional%20reactive%20programming%2C%20smelly%20old%20socks&cmpt=q&tz=). If you've been busy working, you may not have tried it out yet. Or you might have read a little about it. And then you may have been confused. I was. I did. I was again.
...
This means we need to know which rows are on screen. Let's reason backwards from the UI events the browser can give us. We know we're going to be interested in both scrolling and resizing--scrolling because this tells us which set of rows the user is looking at, and resizing because screen size tells us how many rows will fit on screen.

Let's do some bacon. We'll create streams for the scrolling and resizing:
...
Debouncing our resize seems reasonable, so we did that.
...
Actually, all we care about are the `y` position and screen height, so let's simplify:
